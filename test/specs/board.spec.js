...
describe(`POST - Create a board`, () => {
  before(async () => {
    response = await request(API.url)
      .post(`/1/boards/?name=${boardName}&key=${API.key}&token=${API.token}`)
...
describe("GET - Read a board", () => {
  before(async () => {
    response = await request(API.url)
      .get(`/1/boards/${boardId}?key=${API.key}&token=${API.token}`)
      .then((response) => response);
  });
...
describe("PUT - Update a board", () => {
  before(async () => {
    ...
    response = await request(API.url)
      .put(
        `/1/boards/${boardId}?name=${boardName}&key=${API.key}&token=${API.token}`,
      )
      .then((response) => response);
  });
...
describe("DELETE - Delete a board", () => {
  before(async () => {
    response = await request(API.url)
      .delete(`/1/boards/${boardId}?key=${API.key}&token=${API.token}`)
      .then((response) => response);
  });
